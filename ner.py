...
ner_model = models.NER_Model_With_POS_Feats(pos_mdl)
ner_model.fit(normalized_train_data)
y_dev_pred = ner_model.predict(dev_sentences, include_previous_pos=False, inflect_missing=False)
y_dev_pred_with_prev = ner_model.predict(dev_sentences, include_previous_pos=True, inflect_missing=True)
# print('NER predictions with POS features')
